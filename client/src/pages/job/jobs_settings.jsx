      ...
      {categories.map(category => (
        <div key={category.id} className="mb-4">
          ...
          >
            ...
            <svg
              className={`w-6 h-6 transition-transform transform ${toggledCategories.includes(category.id) ? 'rotate-90' : ''}`}
          ...
          {toggledCategories.includes(category.id) && (
            <div className="flex flex-wrap">
              {category.choices.map(choice => (
                <button
                  key={choice}
                  className={`mr-2 mb-2 px-4 py-2 border rounded-lg ${selectedChoice === choice ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
